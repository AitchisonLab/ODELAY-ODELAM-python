...
def summarizeExp(dataPath, saveSwitch):
    ...
    if len(spotlayoutPath)==1:
        columnID = 'Strain ID'
        strainID = pd.read_excel(spotlayoutPath[0], sheet_name='Sheet1', header=0, usecols="A:L").set_index('ROI')
        columnList = ['Strain ID','Strain Info 1','Strain Info 2' ]
        for roi in expDict.keys():

            strainInfo = ' '.join(str(strainID.loc[roi][colID]) for colID in columnList) + ' ' + roi
...
def exportcsv(dataPath, roiList=None):
    ...
    # TODO:  Add ability to look for summary data to shorten export tim
...
def readMMConfigFile(filePath):
    ...
    configFiltPath = pathlib.Path(filePath)
    ...
    for row in configList:
        ...
        if row[0] == 'Device':
            ...
            if row[1] in testDict[row[0]].keys():
